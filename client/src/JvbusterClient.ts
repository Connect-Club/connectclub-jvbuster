...
interface Candidate {
    ...
    readonly relAddr: string;
    readonly relPort: number;
...
interface Offer {
    readonly videobridgeId: string;
    readonly conferenceId: string;
    readonly ufrag: string;
    readonly pwd: string;
    readonly fingerprints: Fingerprpint[];
    readonly candidates: Candidate[];
    readonly rtcpMux: boolean;
    readonly sctpConnectionId: string;
    readonly primaryAudioChannel: PrimaryChannel;
    readonly primaryVideoChannel: PrimaryChannel;
    readonly audioChannels: Channel[];
    readonly videoChannels: Channel[];
...
interface Streams {
    [streamId:string]: {
        readonly videoTracks: Set<string>
        readonly audioTracks: Set<string>
    }
}
...
export class JvbusterClient {
    ...
    private readonly lastMedias: Map<string, MediaChannel[]> = new Map();
    private readonly lastAudioChannelIds: Map<string, Set<string>> = new Map();
    private readonly lastVideoChannelIds: Map<string, Set<string>> = new Map();
    private readonly lastStreams: Map<string, Streams> = new Map();
    private readonly lastSessionVersions: Map<string, number> = new Map();

    private readonly pinnedEndpoints: Set<string> = new Set();

    private readonly updateOffersDelayed: DelayedTask;
    private waitingSdpOffersAccept;

    private offers: Offer[] = [];
    private firstAnswerProcessed: boolean = false;

    constructor(
        private readonly address: string,
        private readonly token: string,
        private readonly logger: Logger,
        private readonly onNewEndpointMessage?: ((from: string, message: object) => void) | null,
        private readonly onNewMessageForDataChannel?: ((msg: string) => void) | null,
        private readonly videoBandwidth?: number | null,
        private readonly audioBandwidth?: number | null,
        private readonly onEndpoints?: ((endpoints: string[]) => void) | null,
    ) {
    ...
    private static getSdpOfferMeta(oldStreams:Streams, newStreams:Streams): SdpOfferMeta {
        const result: SdpOfferMeta = {};
    ...
    private static getPrimarySsrcAttributes(channel: PrimaryChannel): string {
        const lines = [];
    ...
    private static getSsrcAttributes(channel: Channel): string {
        const lines = [];
    ...
    private async getOffers(offersResponse: Response): Promise<Offer[]> {
    ...
    private static readonly getApplicationMedia = (offer: Offer, candidateAttributes: string) =>
`m=application 1 DTLS/SCTP 5000\r
c=IN IP4 0.0.0.0\r
...
`;
    ...
    private static readonly getPrimaryAudioMedia = (primaryAudioChannel: PrimaryChannel, candidateAttributes: string, bandwidth?: number | null) =>
`m=audio 1 RTP/SAVPF 111\r
c=IN IP4 0.0.0.0\r
b=AS:${bandwidth || 16}\r
...
`;
    ...
    private static readonly getPrimaryVideoMedia = (primaryVideoChannel: PrimaryChannel, candidateAttributes: string, bandwidth?: number | null) =>
`m=video 1 RTP/SAVPF 100 96\r
c=IN IP4 0.0.0.0\r
...
`;
    ...
    private static readonly getAudioMedia = (audioChannel: Channel, candidateAttributes: string) =>
`m=audio 1 RTP/SAVPF 111\r
c=IN IP4 0.0.0.0\r
a=rtcp:1 IN IP4 0.0.0.0\r
a=sendonly\r
...
`;
    ...
    private static readonly getVideoMedia = (videoChannel: Channel, candidateAttributes: string) =>
`m=video 1 RTP/SAVPF 100\r
c=IN IP4 0.0.0.0\r
a=rtcp:1 IN IP4 0.0.0.0\r
a=sendonly\r
a=rtpmap:100 VP8/90000\r
...
`;
    ...
    private static readonly getEmptyMedia = (type: string) => `m=${type} 0 RTP/SAVPF 0\r\n`;

    private static readonly getHeader = (offer: Offer, sessionId: string, sessionVersion:number, medias: string[]) =>
`v=0\r
...
`;
    ...
    private toOfferSdp(offer: Offer): SdpOffer | null {
        if(!this.needNewOfferSdp(offer)) return null;
        ...
        const originalMedias = new Map<string,MediaChannel>();
        const newAudioChannelIds = new Set<string>();
        ...
        const newVideoChannelIds:Set<string> = new Set();
        ...
        let sessionVersion:number = (this.lastSessionVersions.get(offer.videobridgeId) || 0) + 1;
    ...
    async start(speaker: boolean = true) : Promise<SdpOffer[]> {
    ...
    public async processAnswers(sdps: string[]): Promise<void> {
    ...
    public processDataChannelMessage(data: string): void {
        const evt = JSON.parse(data);
        if (evt.colibriClass === 'EndpointMessage' && evt.from !== this.endpoint) {
            if(this.onNewEndpointMessage) this.onNewEndpointMessage(evt.from, evt.msgPayload);
    ...
    private async updateOffers():Promise<void> {
    ...
    public subscribe(endpoints: string[]): Subscription | undefined {
        this.logger.info('JvbusterClient', `subscribe(endpoints=${endpoints})`);
        if(!this.firstAnswerProcessed || this.waitingSdpOffersAccept) return;
        ...
        const pinnedEndpointsFromSdp = new Set<string>(
            [...this.lastStreams.values()].flatMap(x => Object.keys(x))
        );

        const pinnedUUIDEndpoints: string[] = this.offers
    ...
    private sendPinnedUUIDEndpointsChangedEvent(endpoints: string[]) : void {
    ...
    public sendEndpointMessage(msgPayload: object, to: string = ''): void {
        if(!this.onNewMessageForDataChannel) return;
        const msg = `{
